...
We propose that we can, using cycle detecting algorithms, efficiently find switches between patients and massively reduce the number of people on waitlists by using these algorithms periodically.
...
- *@ch:background* provides background on the Top Trading Cycles mechanism and its application to GP allocation.

...
The number of patients each GP has varies but on average each GP has about a thousand patients @helfo2024.
...
If the GP they want to switch to does not have any more capacity for new patients, the person has to sign up in a queue and wait for a free slot to open up.
...
Where only 7 out of 268 GPs had available capacity for new patients.
Thus 97% of the GPs in Bergen had no extra capacity @ssb12005.

When a person wants to switch to a GP that has no excess capacity they must sign up and then wait in line.
Open slots are allocated on a first come first serve basis.
In December 2025 there were about 300,000 people waiting for GPs in Norway @helfo2024.
As there were only 5720 GPs each one has on average more than 50 people on his or hers waiting list.
Since there were a relatively large number of people who are waiting to either switch or to be allocated a GP it follows that it is likely that there are cycles, in that a person $P_A$ wants GP $D_A$ which currently has $P_B$ as a patient, but $P_B$ wants to switch to GP $D_B$ who currently has $P_A$ as a patient.
In this case patient $P_A$ ends up waiting for an open slot with $P_B$'s GP and vice versa, but logically instead of waiting $P_A$ and $P_B$ could just swap GPs.
